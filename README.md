# Portafolio

Sitio estático (HTML/CSS/JS) para GitHub Pages. Enfoque: diseño de contenido para redes sociales.

## Estructura
- `index.html`: página única.
- `assets/css/variables.css`: tokens (color, tipografía, espaciado, forma).
- `assets/css/styles.css`: estilos.
- `assets/js/main.js`: menú móvil y filtro de piezas.
- `assets/img/cuentas/`: portadas de cada cuenta.
- `assets/img/piezas/`: posts (4:5), historias y reels (9:16).
- `assets/img/otros/`: ilustración, diagramación, Biblián Vive.

## Pendientes
Buscar `[COMPLETAR]` y `[...]` en `index.html`.
Exportar imágenes en WebP, máximo ~400 KB cada una.
